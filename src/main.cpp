...
int main() {
    ...
    const int frameCount = 36; // 36 frames for 360 degrees (10 degrees per frame)
    ...
    double viewportLength = viewportHeight; // Camera viewport length (depth)
    ...
    Camera camera(cameraPosition, viewportWidth, viewportLength, cameraDirection);
    ...
    Video video(imageWidth, imageHeight, 30); // 30 FPS
